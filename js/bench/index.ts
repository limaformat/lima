...
function bench(name: string, fn: () => void, iterations: number): void {
	...
	const perOpUs = (ms / iterations) * 1000
	const opsPerSec = 1000 / (ms / iterations)
	console.log(
		`${name.padEnd(60)} ${perOpUs.toFixed(2).padStart(10)} us/op  ${opsPerSec.toFixed(0).padStart(10)} ops/sec`
	)
}
