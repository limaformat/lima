...
export type LimaValue =
	...
	| { kind: 'array'; items: LimaValue[] }
	| { kind: 'mapping'; entries: Map<string, LimaValue> }

export const LNull: LimaValue = { kind: 'null' }
export const LBool = (value: boolean): LimaValue => ({ kind: 'bool', value })
export const LInt = (value: number): LimaValue => ({ kind: 'int', value: value === 0 ? 0 : value })
export const LFloat = (value: number): LimaValue => ({ kind: 'float', value: value === 0 ? 0 : value })
export const LString = (value: string): LimaValue => ({ kind: 'string', value })
export const LInstant = (value: Date): LimaValue => ({ kind: 'instant', value })
 ...
 */
export const countNodes = (v: LimaValue): number => {
	if (v.kind === 'array') return 1 + v.items.reduce((sum, item) => sum + countNodes(item), 0)
	if (v.kind === 'mapping') {
		let sum = 1
		for (const child of v.entries.values()) sum += countNodes(child)
		return sum
	}
	return 1
}
 ...
 */
export const canonicalString = (v: LimaValue): string => {
	switch (v.kind) {
		case 'null': return ''
 ...
 */
export const ingestPartialValue = (
	value: any, partialName: string, path: string, depth = 0, seen: Set<unknown> = new Set()
): LimaValue => {
	...
	if (typeof value === 'boolean') return LBool(value)
	...
	if (value instanceof Date) {
		if (isNaN(value.getTime())) invalid('invalid date')
		const year = value.getUTCFullYear()
		if (year < 1 || year > 9999) invalid(`date year ${year} outside the range 0001-9999`)
	...
	if (Array.isArray(value)) {
		...
		if (proto !== null && proto !== Object.prototype) invalid('unsupported value type')
		const entries = new Map<string, LimaValue>()
