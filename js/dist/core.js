...
const byteLength = (s) => utf8Encoder.encode(s).length;
const checkScalarLimit = (v, line) => {
...
const checkDuplicateKeyMap = (entries, key, line, ctx) => {
    if (!entries.has(key))
        return;
...
const parseDateUTC = (str, strict = false, line = 0) => {
    const invalid = () => {
        if (strict)
            throw new Error(`LIMA: invalid date "${str}" at line ${line}`);
        return null;
    };
    ...
    if (NUMBER_RE.test(str)) {
        ...
        if (isFloatForm(str)) {
            if (!Number.isFinite(n)) {
                if (strict)
                    throw new Error(`LIMA: float value overflows to a non-finite value at line ${line}: "${str}"`);
            }
            else if (n === 0 && !isZeroLiteral(str)) {
                if (strict)
                    throw new Error(`LIMA: non-zero float value underflows to zero at line ${line}: "${str}"`);
            }
            else {
                return LFloat(n);
            }
...
// ─── Scalar / quoting ──────────────────────────────────────────────────────
const unescapeDQ = (s, strict = false, line = 0) => {
    if (!s.includes('\\'))
        return s;
    if (strict) {
 ...
 */
const parseQuotedOrTyped = (raw, ctx, line, topLevel) => {
...
const parseScalarValue = (raw, ctx, line) => {
    const first = raw.charCodeAt(0);
...
const parseFlowSequence = (val, ctx, line) => {
    ...
    return rawItems.map((item) => {
        if (!item) {
            if (ctx.strict)
                throw new Error(`LIMA: empty element in flow sequence at line ${line}`);
...
const parseFlowMapping = (val, ctx, line) => {
    ...
    for (const item of splitFlowItems(inner)) {
        if (!item) {
            if (ctx.strict)
                throw new Error(`LIMA: empty element in flow mapping at line ${line}`);
            continue;
        }
        const colonPos = item.indexOf(': ');
        if (colonPos === -1) {
            if (ctx.strict)
                throw new Error(`LIMA: invalid flow mapping item (missing ": ") at line ${line}: "${item}"`);
            return null;
        }
 ...
 * the first place).
 */
const parseBlock = (lines, startIdx, baseIndent, ctx, baseLine) => {
    let items = null;
    let entries = null;
    ...
    while (idx < lines.length) {
        ...
        if (indent > baseIndent) {
            if (items !== null && pendingItem !== null) {
                ...
                if (colonPos !== -1) {
                    ...
                    if (ctx.strict)
                        throw new Error(`LIMA: unexpected syntax in array item continuation at line ${baseLine + idx}: "${trimmed}"`);
                    idx++;
                }
            }
            else {
                if (ctx.strict)
                    throw new Error(`LIMA: unexpected indentation at line ${baseLine + idx}: "${trimmed}"`);
                idx++;
            }
            continue;
        }
        ...
        if (isList) {
            ...
            if (entries !== null) {
                if (ctx.strict)
                    throw new Error(`LIMA: mixed array and map entries for the same key at line ${baseLine + idx}`);
                idx++;
                continue;
            }
            ...
            if (flowMap !== null) {
                ...
                if (ctx.strict)
                    throw new Error(`LIMA: nested block sequence at line ${baseLine + idx}: "${trimmed}"`);
            ...
            if (items !== null) {
                if (ctx.strict)
                    throw new Error(`LIMA: mixed map and array entries for the same key at line ${baseLine + idx}`);
                idx++;
                continue;
            }
            const colonPos = findKeySep(trimmed);
            if (colonPos !== -1) {
                ...
                if (ctx.strict)
                    throw new Error(`LIMA: indented freetext without a block scalar marker at line ${baseLine + idx}: "${trimmed}"`);
                idx++;
            }
        }
    }
                ...
                if (ctx.strict) {
                    for (let li = 1; li < lines.length; li++) {
                        ...
                        if (!trimmed || trimmed.charCodeAt(0) === 35)
                            continue;
                        throw new Error(`LIMA: indented freetext without a block scalar marker at line ${keyLine(i) + li}: "${trimmed}"`);
                    }
                }
                continue;
            }
