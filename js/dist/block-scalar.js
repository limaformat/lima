...
export const leadingSpaces = (line) => {
    let i = 0;
...
export const trailingSpaceEnd = (line) => {
    let i = line.length;
 ...
 */
export const buildBlockScalar = (bodyLines, keyIndent, pipeLine, ctx, builder) => {
    ...
    for (let i = 0; i < bodyLines.length; i++) {
        ...
        if (ind === bl.length)
            continue; // empty (blank or whitespace-only)
        if (ind <= keyIndent) {
            consumed = i;
            break;
        }
    ...
    const merged = [];
    const mergedSpans = builder.tracksStringSourcePositions ? [] : undefined;
    ...
    if (mergedSpans) {
        ...
        for (let i = 0; i < merged.length; i++) {
            for (const span of mergedSpans[i])
