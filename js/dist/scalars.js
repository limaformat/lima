...
const parseExactIsoSpan = (source, start, end, strict, line) => {
    ...
    if (!dateOnly && !instant)
        return null;
    const digitAt = (offset) => source.charCodeAt(start + offset) - 48;
    ...
    if (d0 < 0 || d0 > 9 || d1 < 0 || d1 > 9 || d2 < 0 || d2 > 9 || d3 < 0 || d3 > 9 ||
        d5 < 0 || d5 > 9 || d6 < 0 || d6 > 9 || d8 < 0 || d8 > 9 || d9 < 0 || d9 > 9)
        return null;
    ...
    if (instant) {
        ...
        if (d11 < 0 || d11 > 9 || d12 < 0 || d12 > 9 || d14 < 0 || d14 > 9 ||
            d15 < 0 || d15 > 9 || d17 < 0 || d17 > 9 || d18 < 0 || d18 > 9)
            return null;
        hour = d11 * 10 + d12;
        minute = d14 * 10 + d15;
        second = d17 * 10 + d18;
...
export const NO_SPAN_VALUE = Symbol('NO_SPAN_VALUE');
/**
 ...
 */
export const parseSimpleScalarSpan = (source, start, end, line, strict, builder) => {
    const length = end - start;
    if (length === 10 || length === 20) {
        const exact = parseExactIsoSpan(source, start, end, strict, line);
        if (exact !== null)
            return builder.instant(exact, line);
    }
    if (length === 1 && source.charCodeAt(start) === 126)
    ...
    if (length === 5 && source.charCodeAt(start) === 102 && source.charCodeAt(start + 1) === 97 &&
        ...
        source.charCodeAt(start + 4) === 101)
        return builder.bool(false, line);
    if (length === 0 || length > 15)
        return NO_SPAN_VALUE;
    const first = source.charCodeAt(start);
    if (first === 48 && length !== 1)
        return NO_SPAN_VALUE;
    let value = 0;
 ...
 */
const isUnquotedKeyStart = (c) => (c >= 97 && c <= 122) || (c >= 65 && c <= 90) || (c >= 48 && c <= 57) || c === 95;
const isUnquotedKeyContinue = (c) => isUnquotedKeyStart(c) || c === 58 || c === 45;
/**
...
export const isValidKey = (raw) => {
    ...
    if (!isUnquotedKeyStart(f))
        return false;
    for (let i = 1; i < raw.length; i++) {
        if (!isUnquotedKeyContinue(raw.charCodeAt(i)))
            return false;
    }
    return true;
};
