...
import { parseCoreWithPositions, toPlainValue, NESTING_DEPTH_LIMIT, } from './core.js';
...
const getNestedValueP = (root, path) => {
    ...
    let cur = root.get(parts[0]);
    for (let i = 1; i < parts.length; i++) {
        if (cur === undefined || cur.kind !== 'mapping')
            return undefined;
 ...
 */
const resolveTree = (node, lookup, partials, ctx) => {
    ...
    if (node.kind !== 'string' && isReferenceFreeP(node))
        return node;
    if (node.kind === 'string') {
        if (node.quoted)
            return node;
    ...
    if (node.kind === 'mapping') {
        ...
        for (const [k, c] of node.entries)
            entries.set(k, resolveTree(c, lookup, partials, ctx));
...
export const parseReferences = (frontMatter, options) => {
    ...
    const partialsPositioned = new Map();
    ...
    if (!hasRefs) {
        ...
        // enumeration order).
        const originalPureRefText = new Map();
